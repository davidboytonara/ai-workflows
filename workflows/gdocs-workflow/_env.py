#!/usr/bin/env python3
"""Shared venv bootstrap + runner for Google Docs workflow scripts.

Uses the shared Casper venv at `$HOME/.agents/.venv` with Google API client deps
required by bundled Google Docs scripts and workflow-local auth helpers.
...
def bootstrap(force: bool = False) -> None:
    ...
    try:
        ...
        print(f"gdocs venv bootstrap failed: {exc}", file=sys.stderr)
        sys.exit(3)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Google Docs workflow environment helper.",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
