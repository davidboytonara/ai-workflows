#!/usr/bin/env python3
"""Shared venv bootstrap + runner for Gmail workflow scripts.

Uses the shared Casper venv at `$HOME/.agents/.venv` with Google API client deps
required by workflow-local auth and Gmail scripts.
...
GMAIL_SCRIPT_DIR = SCRIPT_DIR / "scripts"
...
def _venv_ready() -> bool:
    ...
    probe = subprocess.run(
        [
            str(VENV_PY),
            "-c",
            "import googleapiclient.discovery, bs4",
        ],
        capture_output=True,
        text=True,
    )
...
def bootstrap(force: bool = False) -> None:
    ...
    try:
        ...
        print(f"gmail venv bootstrap failed: {exc}", file=sys.stderr)
        sys.exit(3)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Gmail workflow environment helper.",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
    ...
    if args.paths:
        ...
        print(f"gmail_script_dir={GMAIL_SCRIPT_DIR}")
