...
**Turning a task into a casper goal.** This workflow only creates/reads/updates/comments/attaches — it never plans or executes the work a task describes. When a task should turn into an approved `casper` run, use [`../goal-authoring/goal-authoring.md`](../goal-authoring/goal-authoring.md) to draft `$HD/goal.md` from the task's title/description/comments, then hand off to `../casper/casper.md`. A "product story" task's own `# Acceptance Criteria` section is the extraction guide's primary source for that step.
