#!/usr/bin/env python3
"""Shared venv bootstrap + runner for Google Sheets workflow scripts.

Uses the shared Casper venv at `$HOME/.agents/.venv` with Google API client deps
required by workflow-local auth and Google Sheets scripts.
...
SHEETS_SCRIPT_DIR = SCRIPT_DIR / "scripts"
...
def _venv_ready() -> bool:
    ...
    probe = subprocess.run(
        [
            str(VENV_PY),
            "-c",
            "import googleapiclient.discovery, google_auth_oauthlib.flow",
        ],
        capture_output=True,
        text=True,
    )
...
def bootstrap(force: bool = False) -> None:
    ...
    try:
        ...
        print(f"gsheet venv bootstrap failed: {exc}", file=sys.stderr)
        sys.exit(3)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Google Sheets workflow environment helper.",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
    ...
    if args.paths:
        ...
        print(f"sheets_script_dir={SHEETS_SCRIPT_DIR}")
